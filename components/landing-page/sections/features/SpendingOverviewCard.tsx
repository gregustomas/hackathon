...
export function SpendingOverviewCard() {
  ...
  return (
    <div ref={ref} className="bg-white border border-zinc-200 rounded-[2rem] col-span-1 md:col-span-3 p-6 md:p-10 flex flex-col justify-between gap-6 min-h-80 md:min-h-95">
      <div className="flex flex-col gap-1">
        <p className="text-xs text-zinc-400 uppercase tracking-widest font-medium">This week</p>
        <p className="text-2xl md:text-3xl font-semibold tracking-tight text-zinc-950">4 820 CZK</p>
      ...
      <div className="flex items-end gap-1.5 md:gap-2" style={{ height: 80 }}>
        {weekBars.map((b, i) => (
          <div key={b.day} className="flex-1 flex flex-col items-center gap-1.5">
            ...
            <span className={`text-[9px] md:text-[10px] font-medium ${i === 3 ? "text-zinc-950" : "text-zinc-400"}`}>{b.day}</span>
          </div>
        ))}
      </div>

      <div className="grid grid-cols-2 gap-3 border-t border-zinc-100 pt-4 md:pt-6">
        <div>
