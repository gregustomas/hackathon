...
export function CardLimitForm({ cardId, cardNumber, currentPaymentLimit, currentAtmLimit }: CardLimitFormProps) {
const [paymentLimit, setPaymentLimit] = useState<number>(currentPaymentLimit || 5000)
    ...
    const hasChanges = paymentLimit !== currentPaymentLimit || atmLimit !== currentAtmLimit;

    const handleSave = () => {
        startTransition(async () => {
            const result = await updateCardLimits(cardId, paymentLimit, atmLimit)
            
            if (result.error) {
                toast.error(result.error)
            } else {
                toast.success(`Limity pro kartu ${maskedCard} byly úspěšně aktualizovány.`)
            }
        })
    }
    ...
    return (
        <div className="flex flex-col gap-6 bg-card/50 shadow-sm hover:shadow-md p-5 border rounded-xl transition-all">
            ...
            {/* OVLÁDÁNÍ: Limit pro platby */}
            <div className="space-y-4">
                <div className="flex justify-between items-end">
                    <div className="flex items-center gap-2">
                        <ShoppingBag className="w-4 h-4 text-muted-foreground" />
                        <label className="font-medium text-sm">Platby v obchodech a na internetu</label>
                    </div>
