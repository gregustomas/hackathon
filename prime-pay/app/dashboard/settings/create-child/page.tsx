...
import {
  ...
  CreateChildAccountFormValues,
} from "@/schemas/create-child-acc";
...
export default function CreateChildAccountPage() {
  ...
  const form = useForm<CreateChildAccountFormValues>({
    ...
    defaultValues: {
      ...
      dailyLimit: "",          // string – odpovídá schématu
      consent: false,
    },
    mode: "onTouched",
  });
  ...
  const onSubmit: SubmitHandler<CreateChildAccountFormValues> = async (data) => {
    setIsLoading(true);
    try {
      // převedeme dailyLimit na číslo před odesláním
      const dailyLimitNumber = Number(data.dailyLimit);
      if (Number.isNaN(dailyLimitNumber)) {
        // bezpečnostní brzda – Zod by to už měl chytit
        return;
      }

      const formDataObj = new FormData();
      Object.entries({ ...data, dailyLimit: dailyLimitNumber }).forEach(
        ([key, value]) => {
          if (value !== undefined) {
    ...
    <div className="flex justify-center items-center py-12 cs-container">
      <Card className="w-full max-w-2xl">
        <CardHeader>
          ...
          <CardDescription>
            Kompletní bankovní registrace pro nezletilého člena rodiny.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <form id="child-account-form" onSubmit={handleSubmit(onSubmit)}>
            ...
            {step === 1 && (
              <div className="space-y-4 animate-in fade-in">
                <h3 className="flex items-center font-semibold text-lg">
                  <User className="mr-2 size-5" /> Osobní údaje a kontakt
                </h3>
            ...
            {step === 2 && (
              <div className="space-y-4 animate-in fade-in">
                <h3 className="flex items-center font-semibold text-lg">
                  <Home className="mr-2 size-5" /> Adresa bydliště
                </h3>
            ...
            {step === 3 && (
              <div className="space-y-4 animate-in fade-in">
                <h3 className="flex items-center font-semibold text-lg">
                  <Wallet className="mr-2 size-5" /> Parametry účtu
                </h3>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground text-xs">
                    Limit odchozích plateb. Lze kdykoliv později změnit v
                    nastavení karet a účtů.
                  </p>
                </div>
              </div>
            )}
            ...
            {step === 4 && (
              <div className="space-y-6 animate-in fade-in">
                <h3 className="flex items-center font-semibold text-lg">
                  <ShieldCheck className="mr-2 size-5" /> Právní doložka
                </h3>
                <div className="bg-muted p-4 rounded-lg text-sm">
                  Jako zákonný zástupce nesete plnou odpovědnost za tento dětský
                  účet. Tento proces vytvoří plnohodnotný oddělený účet v
                  českých korunách.
                </div>
                ...
                <div className="flex items-start space-x-3 p-4 border rounded-md">
                  ...
                  <div className="space-y-1 leading-none">
                    ...
                    >
                      Prohlašuji, že jsem jedním z rodičů/zákonných zástupců, a
                      souhlasím se založením podřízeného dětského účtu.
                    </Label>
                    {errors.consent && (
            ...
            {step === 5 && successData && (
              <div className="py-8 text-center animate-in zoom-in-95">
                <div className="bg-green-100 mx-auto mb-4 p-3 rounded-full w-fit">
                  <Check className="size-8 text-green-600" />
                </div>
                <h3 className="mb-2 font-bold text-2xl">
                  Účet úspěšně založen!
                </h3>
                <p className="mb-6 text-muted-foreground">
                  Účet byl provázán s vaším účtem.
                </p>
                <div className="bg-muted mx-auto p-4 rounded-lg max-w-sm font-mono text-sm text-left">
                  <p>
                    <strong>Login:</strong> {successData.email}
                  </p>
                  <p>
                    <strong>Heslo:</strong> {successData.pass}
                  </p>
                </div>
              </div>
            )}
        ...
        {step < 5 && (
          <CardFooter className="flex justify-between pt-6 border-t">
            ...
            {step < 4 ? (
              <Button onClick={handleNextStep} type="button" disabled={isLoading}>
                Další krok <ArrowRight className="ml-2 size-4" />
              </Button>
            ) : (
              <Button
                type="submit"
                form="child-account-form"
                disabled={isLoading || watch("consent") !== true}
              >
                {isLoading ? (
                  <>
                    <Loader2 className="mr-2 size-4 animate-spin" /> Vytvářím...
                  </>
                ) : (
                  "Založit účet"
                )}
